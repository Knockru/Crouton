...
export class Twtr {
  ...
  public async verify(): Promise<{ id: string; screen_name: string }> {
    try {
      const response = await this.twtr.get("account/verify_credentials", {});
      return {
        id: response["id_str"] as string,
        screen_name: response["screen_name"] as string
      };
    } catch (e) {
      console.error(e);
      return null;
  ...
  public async update(status: string): Promise<boolean> {
    try {
      await this.twtr.post(`statuses/update`, { status });
      return true;
    } catch (e) {
      console.error(e);
      return false;
